...
Libtest(){
	...
	if [ "$libAddr" == "$GIT" ];then
		libAddr='https://raw.githubusercontent.com/cangshuking/ss-panel-and-ss-py-mu_cangshu/master/libsodium-1.0.13.tar.gz'
	else
...
install_node(){
	...
	echo "# Github: https://github.com/cangshuking/ss-panel-and-ss-py-mu_cangshu   #"
	echo "# Author: 91vps                                             #"
	echo "#############################################################"
	echo
	#Check Root
	...
	wget -N -P  /etc/ --no-check-certificate  https://raw.githubusercontent.com/cangshuking/ss-panel-and-ss-py-mu_cangshu/master/supervisord.conf
	supervisord
	#iptables
	iptables -F
	iptables -X  
...
install_panel_and_node(){
	...
	sed -i '$a * hard nofile 512000\n* soft nofile 512000' /etc/security/limits.conf
	install_centos_ssr
	wget -N -P  /root/shadowsocks/ --no-check-certificate  https://raw.githubusercontent.com/cangshuking/ss-panel-and-ss-py-mu_cangshu/master/userapiconfig.py
	# 启用supervisord
